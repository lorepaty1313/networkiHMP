...
"""
network_mutual_info_ordered_from_snippet.py

Ordered + cleaned script version of the exact workflow you pasted.
...
3) Global variables
4) Read paths
5) Write paths (optional)
6) Read
7) Output (compute + prints + plots)
8) Write out (optional)

Goal: do the same thing, but without duplicate calculations / duplicate function definitions.
"""
...
def build_MI_continuous(
    ...
    # 5) MI (micro vs cada gen) -- PARALLEL
...
# 3) GLOBAL VARIABLES
# =========================
DO_PLOTS = False  # set True to build/plot networks
...
def main() -> None:
    ...
    # alinear A/B global (solo para prints; NO se usa para MI final)
    ...
    if DO_PLOTS:
            ...
            plot_hairball_color(G_CD, "CD hairball", k=HAIRBALL_K, seed=HAIRBALL_SEED, label_micro=True, label_tx=True)

            plot_hairball_color(G_UC, "UC hairball", k=HAIRBALL_K, seed=HAIRBALL_SEED, label_micro=True, label_tx=True)

            plot_hairball_color(G_non, "nonIBD hairball", k=HAIRBALL_K, seed=HAIRBALL_SEED, label_micro=True, label_tx=True)

    else:

        print('DO_PLOTS=False -> skipping network graphs/plots')
